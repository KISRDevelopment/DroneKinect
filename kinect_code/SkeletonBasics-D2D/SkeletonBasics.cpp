...
int SendKinectData::sendData(D2D1_POINT_2F lelbow,
	...
	char buf[512];
	float pitch, roll, deltahand;
	
	pitch = .5*((relbow.y - rhand.y) + (lelbow.y - lhand.y));
	
	roll = (rhand.y - lhand.y) / (rhand.x - lhand.x);
	
	deltahand = rhand.x - lhand.x;
	
	sprintf_s(buf,512, "\2%f,%f,<%f>\3", pitch,roll, deltahand); // coping the data to the buf to s
